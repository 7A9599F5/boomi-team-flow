...
class GetDhToken(BaseStep):
    ...
    def _collect_credentials(self, state: SetupState) -> tuple[str, str]:
        """Prompt user for DataHub Repository API username and token."""
    ...
    def _apply_credentials(self, state: SetupState, username: str, token: str) -> None:
        """Store credentials in both state and in-memory config."""
        state.update_config({"datahub_user": username, "datahub_token": token})
        self.config.hub_auth_user = username
    ...
    def execute(self, state: SetupState, dry_run: bool = False) -> StepStatus:
        ...
        if existing_token and existing_user:
            # Sync in-memory config with state values
            self.config.hub_auth_user = existing_user
            self.config.hub_auth_token = existing_token

            # Verify against the actual Repository API (not Platform API)
            if self.datahub_api.verify_repo_auth():
                ui.print_success("DataHub credentials verified against Repository API")
                return StepStatus.COMPLETED
            else:
                ui.print_warning(
                    "Existing DataHub credentials returned 401 — "
                    "token may have expired or been regenerated"
                )
                username, token = self._collect_credentials(state)
                self._apply_credentials(state, username, token)
        else:
            username, token = self._collect_credentials(state)
            self._apply_credentials(state, username, token)

        # Verify the new credentials against the Repository API
        if self.datahub_api.verify_repo_auth():
            ui.print_success("DataHub credentials verified against Repository API")
        else:
            ui.print_warning(
                "Could not verify credentials via Repository API "
                "(may still work — check username is the generated value, not your account ID)"
            )
