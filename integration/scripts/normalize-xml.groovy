...
Logger logger = Logger.getLogger("normalize-xml")
 ...
 * Output: Normalized XML with consistent indentation (2-space indent via XmlUtil.serialize)
 *
 * Note: XmlUtil.serialize() normalizes whitespace and indentation but does NOT sort attributes.
 *       Attribute order differences between API calls may still appear in diffs.
 */
...
try {
    for (int i = 0; i < dataContext.getDataCount(); i++) {
        ...
        try {
            ...
            logger.severe("Failed to parse XML for normalization: ${e.message}")
            // Return error placeholder instead of raw/broken XML
            String errorXml = "<error>Failed to parse XML: ${e.message}</error>"
            dataContext.storeStream(new ByteArrayInputStream(errorXml.getBytes("UTF-8")), props)
        }
    }
} catch (Exception e) {
    logger.severe("normalize-xml failed: " + e.getMessage())
    throw new Exception("normalize-xml failed: " + e.getMessage())
}
